...
class BtcTransaction {
  ...
  hashForWitnessV0(
    ...
    writeSlice(slice) {
      ...
      tbuffer.setRange(toffset, toffset + length, slice);
      toffset += length;
    }
    ...
    if ((hashType & 0x1f) != SIGHASH_SINGLE &&
        (hashType & 0x1f) != SIGHASH_NONE) {
      var txOutsSize = outs.fold(0, (sum, output) {
        int s = sum as int;
        return s + 8 + varSliceSize(output.script!);
      });
  ...
  hashForSignature(int inIndex, Uint8List prevOutScript, int hashType) {
    ...
    final txTmp = BtcTransaction.clone(this);
  ...
  factory BtcTransaction.clone(BtcTransaction _tx) {
  ...
  factory BtcTransaction.fromBuffer(
    Uint8List buffer, {
    bool noStrict = false,
  }) {
    int offset = 0;
  ...
  factory BtcTransaction.fromHex(
    String hex, {
    bool noStrict = false,
  }) {
    return BtcTransaction.fromBuffer(
...
class Output {
  ...
  factory Output.clone(Output output) {
    return new Output(
      script: output.script != null ? output.script : null,
      value: output.value,
      valueBuffer: output.valueBuffer != null ? output.valueBuffer : null,
      pubkeys: output.pubkeys,
