...
class TransactionBuilder {
  ...
  TransactionBuilder({NetworkType? network, int? maximumFeeRate}) {
    ...
    this._inputs = [];
    this._tx = new BtcTransaction();
    this._tx!.version = 2;
  }

  List<Input>? get inputs => _inputs;

  factory TransactionBuilder.fromTransaction(BtcTransaction transaction,
      [NetworkType? network]) {
  ...
  int addInput(dynamic txHash, int vout,
    ...
    if (txHash is String) {
      ...
      hash = txHash;
    } else if (txHash is BtcTransaction) {
  ...
  BtcTransaction _build(bool allowIncomplete) {
    if (!allowIncomplete) {
      if (_tx!.ins.length == 0)
    ...
    final tx = BtcTransaction.clone(_tx!);
